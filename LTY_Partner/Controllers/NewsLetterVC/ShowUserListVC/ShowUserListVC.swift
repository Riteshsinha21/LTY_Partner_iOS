...
//                self.customerListArr.insert(info, at: sender.tag)
    ...
    func SendNewsLetterData(data: SuccessModel) {
        ...
        if data.status == "SUCCESS" {
            DispatchQueue.main.async {
                self.showOkAlertWithHandler("Sent successfully.".localized()) {
                    self.popVC(animated: true)
                }
            }
        }
...
extension ShowUserListVC : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        footerView.btnSend.setTitle("Send".localized(), for: .normal)
    ...
    @objc func buttonFooterAction(_ sender:UIButton){
        print(sender.tag)
        if tempId == ""{
            self.ShowAlert(message: "Please choose template.".localized())
            return
        }else if self.userList.count == 0{
            self.ShowAlert(message: "Please select User.".localized())
            return
        }
        self.sendTemplate()
