    ...
    func setData(data: BusinessLegalProModel) {
        ...
        if data.status == "SUCCESS"{
            ...
            //    self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                self.lblProposalType.text =  responseData?.metadata?.proposalFor ?? ""
               ...
               // self.lblphone.text = companyDetails?.phone ?? ""
                ...
                self.lblSatrtOfContract.text = returnDOB(date: companyDetails?.startOfContract ?? "")
                self.lblEffectiveDate.text = returnDOB(date: companyDetails?.effectiveDate ?? "")
              
                self.lblInsuredCurrently.text = responseData?.metadata?.involvedInLitigation ?? ""
                self.lblStartDate.text = returnDOB(date:responseData?.metadata?.startDate ?? "")
                self.lblEndDate.text = returnDOB(date:responseData?.metadata?.endDate ?? "")
                self.lblExpiryDate.text = returnDOB(date:responseData?.metadata?.expireDate ?? "")
                
                self.lblInsuredPerson.text = responseData?.metadata?.insuredForLegal ?? ""
                
                if (self.lblInsuredPerson.text!.caseInsensitiveCompare("yes".localized()) == .orderedSame) {
