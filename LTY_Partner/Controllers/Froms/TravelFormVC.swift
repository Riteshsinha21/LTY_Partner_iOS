...
class TravelFormVC: UIViewController {
    ...
    @IBOutlet weak var personsToBeInsuredlblTitle: UILabel!
    @IBOutlet weak var lblTitleFullName: UILabel!
    @IBOutlet weak var phonelblTitle: UILabel!
    @IBOutlet weak var addresslblTitle: UILabel!
    @IBOutlet weak var doblblTitle: UILabel!
    @IBOutlet weak var nationalitylblTitle: UILabel!
    @IBOutlet weak var durationlblTitle: UILabel!
    @IBOutlet weak var effectiveDatelblTitle: UILabel!
    @IBOutlet weak var desiredCoverslblTitle: UILabel!
    ...
    @IBOutlet weak var personsToBeInsuredLbl: UILabel!
    ...
    @IBOutlet weak var desiredCoversLbl: UILabel!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        leadInsuranceViewModel.travelDelegate = self
    ...
    func setLocalization(){
        ...
        self.phonelblTitle.text = "Phone Number".localized()
        self.addresslblTitle.text = "Address".localized()
        self.doblblTitle.text = "Date of Birth".localized()
        self.nationalitylblTitle.text = "Nationality/License ".localized()
        self.durationlblTitle.text = "Duration".localized()
        self.effectiveDatelblTitle.text = "Effective Date ".localized()
        self.desiredCoverslblTitle.text = "Cover Type".localized()
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        leadInsuranceViewModel.TravelInsuranceApiCall(param: param)
...
extension TravelFormVC : SetTravelInsuranceData {
        ...
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
               ...
               // self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                let basicInfoTitle =  basicInfo?.title?.capitalized   ?? ""
                let fname =  basicInfo?.firstName?.capitalized   ?? ""
                let lname =  basicInfo?.lastName?.capitalized    ?? ""
                
                self.lblFullName.text = basicInfoTitle + " " + fname + " " + lname
                
                let countryCode12 = basicInfo?.countryCode ?? ""
                self.phoneLbl.text = "\(countryCode12) \(basicInfo?.mobile ?? "")"
             //   self.phoneLbl.text = basicInfo?.mobile  ?? ""
                self.addressLbl.text = basicInfo?.address  ?? ""
                self.nationalityLbl.text = basicInfo?.country  ?? ""
                self.dobLbl.text = returnDOB(date:basicInfo?.dob  ?? "")
                self.nationalityLbl.text = basicInfo?.country  ?? ""
                self.durationLbl.text = data.data?.metadata?.personalDetails?.duration ?? ""
                self.effectiveDateLbl.text = data.data?.metadata?.personalDetails?.effectiveDate ?? ""
                self.personsToBeInsuredLbl.text = data.data?.metadata?.personsToBeInsured ?? ""

                if(self.personsToBeInsuredLbl.text!.caseInsensitiveCompare("family") == .orderedSame) {
                    self.adultCountView.isHidden = false
                    self.childrenCountView.isHidden = false
                } else {
                    self.adultCountView.isHidden = true
                    self.childrenCountView.isHidden = true
                    
                }
                self.adultCountLbl.text = "\(data.data?.metadata?.adultsCount ?? 0)"
                self.childrenCountLbl.text = "\(data.data?.metadata?.childrenCount ?? 0)"
                var desiredCoversArr = data.data?.metadata?.desiredCovers ?? []
                var arr = NSMutableArray()
                for item in desiredCoversArr {
                    arr.add(item)
                }
                self.desiredCoversLbl.text = arr.componentsJoined(by: ", ")
...
extension TravelFormVC : CategoryListProtocolDelegate, CategoryListByAgentProtocolDelegate {
