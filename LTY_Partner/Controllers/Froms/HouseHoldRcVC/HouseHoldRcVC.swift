...
class HouseHoldRcVC: UIViewController {
    ...
    func setUpLocalization() {
        ...
        self.titleLblBonusProtection.text = "PLACE OF RISK".localized()
       ...
       // self.titleLblRC.text = "Occasional use of motor vehicles belonging to third parties:".localized()
        self.titleLblRC.text = "Automotive".localized()
        ...
        self.titleLblClaims.text = "Specify the Claims".localized()
        ...
        self.titleLblcopmpanyNamePolicy.text = "Previous Company ?".localized()
        self.titleLblNonInsurance.text = "What is the reason for this non-insurance (if age over 35)?".localized()
        self.titleLblSpecifyClaim.text = "Have you suffered any claims in the proposed branches?".localized()
    }
        ...
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
             ...
             //   self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                self.lblEffectiveDate.text = returnDOB(date:responseData?.effectiveDate ?? "")
                self.lblSumInsured.text = metaData?.sumInsured?.html2String ?? ""
                self.lblConstruction.text = metaData?.typeOfConstruction ?? ""
                self.lblCivil.text = metaData?.civilLiability ?? ""
                ...
                self.lblBonusProtection.text = metaData?.placeOfRisk ?? ""
                if (self.lblBonusProtection.text!.caseInsensitiveCompare("Other".localized()) == .orderedSame) {
                    self.descriptionView.isHidden = false
                    self.descriptionLbl.text = metaData?.placeOfRiskDescription ?? ""
                } else {
                ...
                if (self.lblClaims.text!.caseInsensitiveCompare("Yes".localized()) == .orderedSame)  {
