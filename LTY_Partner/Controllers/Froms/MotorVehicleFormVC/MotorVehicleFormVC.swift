...
class MotorVehicleFormVC: UIViewController {
    ...
    @IBOutlet weak var lblNameInstitution: UILabel!
    @IBOutlet weak var lblTitleInstitution: UILabel!
    
    @IBOutlet weak var contractView: UIView!
    @IBOutlet weak var lblContractNumber1: UILabel!
    @IBOutlet weak var lblTitleContractNumber: UILabel!
    
    @IBOutlet weak var plateNumberView: UIView!
    @IBOutlet weak var lblBreakDownService: UILabel!
    @IBOutlet weak var lbltitleBreakDownService: UILabel!
    
    @IBOutlet weak var trafficAccidentInView: UIView!
    @IBOutlet weak var lblTrafficAccidentInTitle: UILabel!
    @IBOutlet weak var lblTraficAccidentIn: UILabel!
    ...
    @IBOutlet weak var lblDriverNationality: UILabel!
    @IBOutlet weak var lblDriverDob: UILabel!
    @IBOutlet weak var titleDriverDob: UILabel!
    @IBOutlet weak var titleDriverNationality: UILabel!
    @IBOutlet weak var driverDobView: UIView!
    @IBOutlet weak var driverNationalityView: UIView!
    @IBOutlet weak var collisionDeductView: UIView!
    @IBOutlet weak var lblCollisionDeduct: UILabel!
    @IBOutlet weak var collisionDeducTitle: UILabel!
    @IBOutlet weak var lblCascoIntegral: UILabel!
    @IBOutlet weak var cascoIntgeralTitle: UILabel!
    @IBOutlet weak var partialCascoTitle: UILabel!
    @IBOutlet weak var lblPlateNum: UILabel!
    @IBOutlet weak var lblTitlePlateNo: UILabel!
    ...
    @IBOutlet weak var lblPartialCasco: UILabel!
    ...
    func setLocalization() {
        ...
        self.lblTitleInstitution.text = "Name of Institution".localized()
        self.lblTitleContractNumber.text = "Contract Number".localized()
        self.lbltitleBreakDownService.text = "Breakdown Service".localized()
        self.lbltitleInsuranceCompany.text = "Insurance Company".localized()
        self.lblTitlePlateNo.text = "Plate Number".localized()
        self.titleDriverDob.text = "Driver Dob".localized()
        self.titleDriverNationality.text = "Driver Nationality".localized()
        ...
        self.partialCascoTitle.text = "PARTIAL CASCO".localized()
        self.cascoIntgeralTitle.text = "CASCO INTEGRAL".localized()
        self.collisionDeducTitle.text = "Collision Deductible".localized()
        self.partialCascoTitle.text = "PARTIAL CASCO".localized()
    ...
    @IBAction func viewGrayDocActionBtn(_ sender: UIButton) {
        print("view grayCard doc")
//        let vc = ViewImageVC.instantiate(fromAppStoryboard: .proposalStoryboard)
//        vc.image = grayCardDocUrl
        let vc = ViewAttachmentsVC.instantiate(fromAppStoryboard: .home)
        vc.attachmentStr = grayCardDocUrl
    ...
    @IBAction func viewAttachDocActionBtn(_ sender: UIButton) {
        print("view attach doc")
//        let vc = ViewImageVC.instantiate(fromAppStoryboard: .proposalStoryboard)
//        vc.image = attcahDocUrl
//        vc.modalTransitionStyle = .crossDissolve
//        vc.modalPresentationStyle = .overFullScreen
//        self.present(vc, animated: true)
        let vc = ViewAttachmentsVC.instantiate(fromAppStoryboard: .home)
        vc.attachmentStr = attcahDocUrl
        ...
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
               ...
               // self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                let basicTitle =  basicInfo?.title?.capitalized   ?? ""
                let fname =  basicInfo?.firstName?.capitalized   ?? ""
                let lname =  basicInfo?.lastName?.capitalized    ?? ""
                self.lblMemberInfoFullName.text = basicTitle + " " + fname + " " + lname
                
                let metadata = data.data?.metadata
                self.lblDriverType.text = metadata?.reqType ?? ""
                self.lblPlateNum.text = metadata?.plateNumber ?? ""
                
                if (self.lblDriverType.text!.caseInsensitiveCompare("Plates Number".localized()) == .orderedSame){
                ...
                let personalTitle =  info?.title?.capitalized   ?? ""
                let personalfname =  info?.firstName?.capitalized   ?? ""
                let personallname =  info?.lastName?.capitalized    ?? ""
                self.lblFullName.text =  personalTitle + " " + personalfname + " " + personallname
                self.lblAddress.text = info?.address ?? ""
                self.lblDob.text = returnDOB(date:info?.dob ?? "")
                self.lblNationality.text = info?.nationality ?? ""
                self.lblDL.text = returnDOB(date: metadata?.drivingLicenseDate ?? "")
                self.lblPaymentMethod.text = metadata?.paymentMethod ?? ""
                self.lblSourceOfFunding.text = metadata?.sourceOfFunding ?? ""
               // self.lblSourceOfFunding.text = metadata?.sourceOfFunding ?? ""
                
                if (self.lblSourceOfFunding.text!.caseInsensitiveCompare("Leasing".localized()) == .orderedSame){
                    
                    self.lblContractNumber1.text = metadata?.contractNumber ?? ""
                    self.lblNameInstitution.text = metadata?.nameOfInstitution ?? ""
                    self.contractView.isHidden = false
                    self.institutionView.isHidden = false
                }else{
                    self.contractView.isHidden = true
                    self.institutionView.isHidden = true
                }
...
//                self.lblNameOfInstitution.text = metadata?.nameOfInstitution ?? ""
                ...
                if(self.lblDrivers.text!.caseInsensitiveCompare("yes".localized()) == .orderedSame){
                    self.driverNameView.isHidden = false
//                    self.driverContactView.isHidden = false
                    self.driverAddressView.isHidden = false
                    self.driverNationalityView.isHidden = false
                    self.driverDobView.isHidden = false
                } else {
                    self.driverNameView.isHidden = true
                    self.driverContactView.isHidden = true
                    self.driverAddressView.isHidden = true
                    self.driverNationalityView.isHidden = true
                    self.driverDobView.isHidden = true
                }
                ...
                self.lblDriverDob.text = returnDOB(date: driverInfo?.dob ?? "")
                self.lblDriverNationality.text = driverInfo?.nationality ?? ""
                ...
                if(self.lblGrayCard.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblGrayCard.text!.caseInsensitiveCompare("Oui") == .orderedSame){
                ...
                let attachDocInfo = metadata?.attachCard as! policyDocDetails
                if let fileName = attachDocInfo.originalFileName {
                    self.lblAttachDoc.text = fileName
                    self.attcahDocUrl = attachDocInfo.key!
                }
                ...
                if(self.vehicle2Graycard.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.vehicle2Graycard.text!.caseInsensitiveCompare("Oui") == .orderedSame){
                ...
                let attachSecondVehicleCardInfo = metadata?.attachSecondVehicleCard as! policyDocDetails
                if let secondFileName = attachSecondVehicleCardInfo.originalFileName {
                    self.vehicle2GrayCardDoc.text = secondFileName
                    self.grayCardDocUrl = attachSecondVehicleCardInfo.key!
                }
                
                
                self.lblVihicleOwner.text = metadata?.areYouInsuredAsVehicleOwner ?? ""
                
                if(self.lblVihicleOwner.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblVihicleOwner.text!.caseInsensitiveCompare("Oui") == .orderedSame) {
                ...
                if self.lblExistingCompany.text == "yes" {
                    self.insuranceView.isHidden = false
                    self.lblInsuranceCompany.text = metadata?.insuranceCompany ?? ""
                }else{
                ...
                if(self.lblDlWithDrawn.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblDlWithDrawn.text!.caseInsensitiveCompare("Oui") == .orderedSame){
                ...
                if(self.lblTrafficaccident.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblTrafficaccident.text!.caseInsensitiveCompare("Oui") == .orderedSame) {
                    self.accidentDateView.isHidden = false
                    self.accidentAmountView.isHidden = false
                    self.accidentResponsibleView.isHidden = false
                    self.trafficAccidentInView.isHidden = false
                } else {
                    self.accidentDateView.isHidden = true
                    self.accidentAmountView.isHidden = true
                    self.accidentResponsibleView.isHidden = true
                    self.trafficAccidentInView.isHidden = true
                }
                
                let trafficAccidentDetails = metadata?.trafficAccidentDetails
                self.trafficAccidentDate.text = returnDOB(date: trafficAccidentDetails?.date ?? "")
                ...
                self.lblTraficAccidentIn.text = trafficAccidentDetails?.trafficAccidentDetailsIn ?? ""
                self.lblDamage.text = metadata?.alreadySufferedDamage ?? ""
                
                if(self.lblDamage.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblDamage.text!.caseInsensitiveCompare("Oui") == .orderedSame){
...
//                for item in damageDetails {
//                    arr.add(item)
//                }
//                self.lblRC.text = coversArr.componentsJoined(by: ", ")
                if damageDetails?.sufferedDamageDetailsIn?.count ?? 0 > 0 {
                    var arr = NSMutableArray()
                ...
                self.lblDamageDate.text = returnDOB(date: damageDetails?.date ?? "")
                ...
                self.lblPartialCasco.text = metadata?.partialCasco ?? ""
                self.lblCascoIntegral.text = metadata?.cascoIntegral ?? ""
                if(self.lblCascoIntegral.text!.caseInsensitiveCompare("with".localized()) == .orderedSame) {
                    self.collisionDeductView.isHidden = false
                    self.lblCollisionDeduct.text = metadata?.collisionDeductible ?? ""
                } else {
                    self.collisionDeductView.isHidden = true
                }
                ...
                if(self.lblPersnalEffect.text!.caseInsensitiveCompare("yes") == .orderedSame) || (self.lblPersnalEffect.text!.caseInsensitiveCompare("Oui") == .orderedSame) {
                ...
                self.lblBreakDownService.text = metadata?.breakdownService ?? ""
