...
class AnimalFormVC: UIViewController {
    ...
    @IBOutlet weak var lblTitleInsuranceFor: UILabel!
    @IBOutlet weak var lblInsuranceFor: UILabel!
    
    @IBOutlet weak var lblConfirmation: UILabel!
    @IBOutlet weak var titleConfirmation: UILabel!
    //MARK:- title outlets
    ...
    func setUpLocalization(){
        ...
        self.lblTitleInsuranceFor.text = "Insurance For".localized()
        
        self.lblTitlePersnalInfoFullName.text = "Full Name".localized()
        self.titleConfirmation.text = "I hereby confirm that my pet is over three months old and under six years old.".localized()
...
//                let responseData = data.data?.metadata?.personalDetails
                ...
                self.lblDob.text = returnDOB(date: metaData?.dob ?? "")
                self.lblEmail.text = metaData?.personalDetails?.email ?? ""
                self.lblColor.text = metaData?.color ?? ""
                self.lblGendr.text = metaData?.gender ?? ""
                self.lblBreed.text = metaData?.breed ?? ""
                self.lblZipCode.text = metaData?.personalDetails?.postCode ?? ""
                self.lblStreetNo.text = metaData?.personalDetails?.streetNo ?? ""
                self.insuranceStartDate.text = returnDOB(date: metaData?.startDate ?? "")
                ...
                self.lblFirstName.text = "\(metaData?.personalDetails?.title?.capitalized   ?? "")" + " " +  "\(metaData?.personalDetails?.firstName?.capitalized   ?? "")"
               ...
               // self.lblPhoneNumber.text = metaData?.personalDetails?.mobile ?? ""
                ...
                self.lblEffectiveDate.text = returnDOB(date:metaData?.personalDetails?.effectiveDate ?? "")
                self.lblMonthlyPremium.text = metaData?.monthlyPremiums ?? ""
                self.lblInsuranceFor.text = metaData?.insuranceFor ?? ""
                self.lblElectronicChipNo.text = metaData?.electronicChipNo ?? ""
                self.lblConfirmation.text = metaData?.confirmation ?? ""
