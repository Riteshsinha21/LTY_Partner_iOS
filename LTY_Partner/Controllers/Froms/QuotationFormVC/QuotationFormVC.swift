...
//MARK:- Title Outlets
    ...
    @IBOutlet weak var lblTitleSpecifyClaim: UILabel!
    ...
    func setLocalization(){
        ...
        self.lblTitleSpecifyClaim.text = "Please spacify the claims".localized()
        self.lblTitleCompanyName.text = "Company Name".localized()
        ...
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
              ...
              //  self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                if (self.lblPastInsured.text!.caseInsensitiveCompare("Yes".localized()) == .orderedSame)  {
                    self.companyView.isHidden = false
                    self.policyView.isHidden = false
                    self.lblCompanyName.text = basicInfo?.companyName ?? ""
                    self.lblPolicyNumber.text = basicInfo?.policyNo ?? ""
                }else{
                    self.companyView.isHidden = false
                    self.policyView.isHidden = false
                }
                ...
                if (self.lblProposedBranch.text!.caseInsensitiveCompare("Yes".localized()) == .orderedSame)  {
                    self.claimView.isHidden = false
                    
                    self.lblSpecifyClaim.text = basicInfo?.claimDetails ?? ""
                }else{
                    self.claimView.isHidden = false
                }
                ...
                let basicInfoTitle =  basicInfo2?.title?.capitalized   ?? ""
                ...
                self.lblDob.text = returnDOB(date: basicInfo2?.dob  ?? "")
                ...
                let info = basicInfo?.supportingDoc!
                if let fileName = info?.originalFileName{
                    self.lblSupportingDoc.text = fileName
                }
                
                
                if self.lblSupportingDoc.text == "" {
                    self.showDocBtn.isHidden = true
                }else{
                    self.showDocBtn.isHidden = false
                    if let fileName1 = info?.originalFileName{
                        debugPrint(fileName1)
                        self.supportingDocUrl = info!.key ?? ""
                    }
                    
                }
                
                self.lblEffectiveDate.text = returnDOB(date: basicInfo2?.effectiveDate  ?? "")
