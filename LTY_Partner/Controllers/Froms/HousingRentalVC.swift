...
class HousingRentalVC: UIViewController {
    ...
    @IBOutlet weak var otherFullNameView: UIView!
    @IBOutlet weak var otherDobView: UIView!
    @IBOutlet weak var otherPhoneView: UIView!
    @IBOutlet weak var otherEmailView: UIView!
    @IBOutlet weak var otherOccupationView: UIView!
    @IBOutlet weak var lblOtherPerson: UILabel!
    @IBOutlet weak var titleOtherPerson: UILabel!
    // MarK:- Title Outlets
...
//
    ...
    func setLocalization () {
        ...
        self.titleOtherPerson.text = "Other persons/applicants indicated in the lease contract 2".localized()
        ...
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
               ...
               // self.lblPersnalInfoMobile.text = persnalInfo?.mobile  ?? ""
                ...
                self.lblMemberInfoFullName.text = basicInfoTitle + " " + fname + " " + lname
                ...
                self.lblStartDate.text = returnDOB(date:metadata?.desiredStart ?? "")
                self.lblpremiumDueDate.text = returnDOB(date: metadata?.premiumDueDate ?? "")
                ...
                self.lblApplicantFullName.text =  "\(tenantData?.title ?? "")" + " " + "\(tenantData?.firstName ?? "")" + " " + "\(tenantData?.lastName ?? "")"
                self.lblApplicantDob.text = returnDOB(date:tenantData?.dob ?? "")
                ...
                self.lblApplicantEffectiveDate.text = returnDOB(date:tenantData?.effectiveDate ?? "")
                self.lblOtherPerson.text = "\(tenantData?.isOtherPersonAvailable ?? true)"
                if(self.lblOtherPerson.text!.caseInsensitiveCompare("true".localized()) == .orderedSame) {
                    self.otherFullNameView.isHidden = false
                    self.otherDobView.isHidden = false
                    self.otherPhoneView.isHidden = false
                    self.otherEmailView.isHidden = false
                    self.otherOccupationView.isHidden = false
                } else {
                    self.otherFullNameView.isHidden = true
                    self.otherDobView.isHidden = true
                    self.otherPhoneView.isHidden = true
                    self.otherEmailView.isHidden = true
                    self.otherOccupationView.isHidden = true
                }
               ...
               // self.lblStewardPhone.text = stewardData?.phoneNo ?? ""
                ...
                let otherPerson = metadata?.otherPersonDetails
                self.lblOtherUserFullName.text =  " \(otherPerson?.title?.capitalized   ?? "")" + " " + "\(otherPerson?.firstName?.capitalized   ?? "")" + " " + "\(otherPerson?.lastName?.capitalized   ?? "")"
...
//                let countryCode12 = ""
              ...
              //  self.lblOtherUserMobile.text = otherPerson?.mobile ?? ""
                self.lblOtherUserDob.text = returnDOB(date:otherPerson?.dob ?? "")
