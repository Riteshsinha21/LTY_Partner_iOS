    ...
    func setData(data: LegalProtectionModel) {
        if data.status == "SUCCESS"{
            DispatchQueue.main.async {
                ...
                if(self.lblCoverType.text!.caseInsensitiveCompare("building".localized()) == .orderedSame){
                ...
                let personalfnameTitle =  info?.title?.capitalized   ?? ""
                let personalfname =  info?.firstName?.capitalized   ?? ""
                let personallname =  info?.lastName?.capitalized    ?? ""
                self.lblFullName.text = personalfnameTitle + " " + personalfname + " " + personallname
                ...
                self.lblEffectiveDate.text = returnDOB(date: info?.effectiveDate ?? "")
