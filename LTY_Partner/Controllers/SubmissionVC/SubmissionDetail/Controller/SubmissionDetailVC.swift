...
protocol submissionListApiProtocol{
    func callApiFunc(setValue:Bool)

}

class SubmissionDetailVC: UIViewController, UITextFieldDelegate {
    
    var submissionListApiDelegate :submissionListApiProtocol?
   ...
   // var policyDocArr = [String]()
    var policyDocArr = NSMutableArray()
    var isPending = false
    var proposalId = ""
    var proposaldetailVM = ProposalDetailViewModel()
    var docArr = [policyDocDetails]()
    var docArrForApi = NSMutableArray()
    var picker = UIImagePickerController()
    var pickedImageUrl:URL?
    var policyDocFileName = ""
        ...
        Services.shareInstance.postRequestApi(url: LTY_BASE_URL.BASE_URL_INSURANCE + LTY_END_POINT_URL.viewProposal, method: "POST", passToken: true, expecting:ViewProposalModel.self, dataDict: param) { response
            ...
            case .success(let result):
                ...
                DispatchQueue.main.async {
                    ...
                    self.lblRelation.text = info?.leadDetails?.relation?.localized() ?? ""
                   ...
                   // self.agentContactLbl.text = info?.agentDetails?.mobile ?? ""
                    ...
                    let signedDocs = info?.signedDoc as! [policyDocDetails]
//                    let signedDocs = info?.signedDoc ?? []
//                    for item in signedDocs {
//                        self.docArr = signedDocs
//                    }
                    self.docArr = signedDocs
        ...
        Services.shareInstance.postRequestApi(url: LTY_BASE_URL.BASE_URL_INSURANCE + LTY_END_POINT_URL.addPolicyId, method: "POST", passToken: true, expecting:ViewProposalModel.self, dataDict: param) { response
            ...
            case .success(let result):
                ...
                if result.status == "SUCCESS" {
                    ...
                    DispatchQueue.main.async {
                        self.showToast(message: "Proposal Sent successfully".localized(), font: FontSize.size12!)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5){
                            self.navigationController?.popViewControllerWithHandler(animated: true, completion: {
                                self.submissionListApiDelegate?.callApiFunc(setValue: true)
                            })
...
//                        self.showOkAlertWithHandler("Proposal Sent successfully".localized()) {
    ...
    @IBAction func btnSubmitAction(_ sender: UIButton) {
        ...
        guard  let UserData = UserDefaults.standard.value(forKey: AppKeys.roleList) as? Data  else {
            ...
            self.docArrForApi.removeAllObjects()
            for item in policyDocArr{
                let dic = NSMutableDictionary()
                let itemDic = item as! policyDocDetails
              //  print(itemDic)
                dic.setValue(itemDic.key, forKey: "key")
                dic.setValue(itemDic.originalFileName, forKey: "originalFileName")
                self.docArrForApi.add(dic)
            }
                ...
                if result.status == "SUCCESS" {
                    DispatchQueue.main.async {
                        if result.data?.isValid == true {
                            let param : [String:Any] = ["proposalId": self.proposalId, "policyId": self.textPolicyId.text!, "commissionDate": self.commissionStartDate, "frequency": self.textPeriodicity.text!, "contractDoc": self.docArrForApi,"needToOverride":self.shouldOverride]
...
//           cell.cellLbl.text = (self.docArr[indexPath.row])
            let info = self.docArr[indexPath.row] as! policyDocDetails
            let fileExtension = info.key as! NSString
            let abc = fileExtension.pathExtension
            cell.cellLbl.text = info.originalFileName!
//            cell.cellLbl.text = "Signed Doc \(indexPath.row + 1).\(abc)"
...
//            cell.cellLbl.text = (self.policyDocArr[indexPath.row])
            let info = self.policyDocArr[indexPath.row] as! policyDocDetails
            cell.cellLbl.text = info.originalFileName!
...
//            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        var filename = NSString()
         if tableView == self.uploadDocTable {
             let info = self.docArr[indexPath.row] as! policyDocDetails
             filename = info.key! as NSString
         } else {
             let info = policyDocArr[indexPath.row] as! policyDocDetails
             filename = info.key! as NSString
...
//            let filename = URL(fileURLWithPath: String(describing:urls)).lastPathComponent
            self.policyDocFileName = URL(fileURLWithPath: String(describing:urls)).lastPathComponent.replacingOccurrences(of: "%[0-9a-fA-F]{2}",with: "",options: .regularExpression,range: nil)
    ...
    //        UIApplication.topViewController()?.dismiss(animated: true, completion: nil)
            ...
            if let imgUrl = getImageUrl()
            {
                ...
                self.policyDocFileName = URL(fileURLWithPath: String(describing:imgUrl)).lastPathComponent.replacingOccurrences(of: "%[0-9a-fA-F]{2}",with: "",options: .regularExpression,range: nil)
            }
    ...
    func requestNativeImageUpload(_ docName: String){
        ...
        ServerClass.sharedInstance.sendMultipartRequestToServer(urlString: fullUrl, fileName: "doc", sendJson: param, imageUrl: pickedImageUrl, successBlock: { (json) in
            ...
            if success  == "SUCCESS"
            {
                self.policyDocArr.removeAllObjects()
                let url = json["data"]["key"].stringValue
                
                let OriginalFileName = json["data"]["originalFileName"].stringValue
                var dic = NSMutableDictionary()
                dic.setValue(url, forKey: "key")
                dic.setValue(OriginalFileName, forKey: "originalFileName")
                let modelDic = policyDocDetails.init(key: url, originalFileName: OriginalFileName)
                self.policyDocArr.add(modelDic)
