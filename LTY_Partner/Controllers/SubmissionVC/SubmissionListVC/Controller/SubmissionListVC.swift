  ...
  //MARK:- Api call func
    ...
    func ApiCallingFunc() {
        ...
        let param: [String : Any] = [ "entityType": entityType as Any,
                                      ...
                                      "pageSize": 10,
                                      "pageCount": self.pageCount, "state": self.state, "agentId": agentId]
        submissionListViewModel.submissionListApiCall(param:param, showLoader: self.pageCount > 1 ? false : true)
    }
...
extension SubmissionListVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblRequestType.text = info.insuranceType?.localized() ?? ""
...
//        cell.lblAddedOn.text = returnDOB(date: lastAddedOn)
        cell.lblAddedOn.text = utcToLocal(dateStr: lastAddedOn)
...
//        cell.lblEnquiredOn.text = returnDOB(date: enquiredOn)
        cell.lblEnquiredOn.text = utcToLocal(dateStr: enquiredOn)
        ...
        if info.proposalState == "SIGNED_DOCS" {
            ...
            cell.lblAccepted.text = "Completed".localized()
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
            if indexPath.row == self.submissionListArr.count - 1, fetchMoreData {
                self.pageCount += 1
                self.ApiCallingFunc()
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.submissionListApiDelegate = self
...
extension SubmissionListVC:submissionListApiProtocol{
    func callApiFunc(setValue: Bool) {
        if setValue == true {
            submissionListArr.removeAll()
            ApiCallingFunc()
        }
    }
