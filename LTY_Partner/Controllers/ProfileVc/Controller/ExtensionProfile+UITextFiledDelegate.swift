//
//  ExtensionProfile+UITextFiledDelegate.swift
//  LTY_Partner
//
...
extension ProfileVC : UINavigationControllerDelegate, UIDocumentMenuDelegate, UIDocumentPickerDelegate, UIImagePickerControllerDelegate {
...
//            alert.addAction(takephoto)
    ...
    //        UIApplication.topViewController()?.dismiss(animated: true, completion: nil)
            if let pickedImage = info[.originalImage] as? UIImage {
                ...
                self.imgUserImage.image = pickedImage
                //self.imgUserImage.sd_setImage(with: self.pickedImageUrl, placeholderImage: UIImage(named: "user"))
            }
    ...
    func profileImageUpload( docName: String){
        ...
        let param = ["userId": "", "profilePic": docName, "uploadedBy": partnerIdd, "memberId": ""]
        print(param)
...
//                let param : [String:Any] = ["key":key ]
//                self.ViewImageApi(param: param, key: key)
                
                DispatchQueue.main.async {
                    self.profileImage = key
                   // kUserDefaults.set(url, forKey: AppKeys.profile)
                   // self.imgUserImage.sd_setImage(with:URL(string: pickedImageUrl), placeholderImage: UIImage(named: "user"))
//                    self.imgUserImage.sd_setImage(with: self.pickedImageUrl, placeholderImage: UIImage(named: "user"))
                }
                ...
                if success  == "SUCCESS"
                {
                    ...
                    DispatchQueue.main.async {
                        if self.selectedTxtFieldTag == 601{
                            ...
                            self.privateCriminalRecord = url
                            
                            let dataPrivateOP = url
                        ...
                        }else if self.selectedTxtFieldTag == 602 {
                            print("privateOP")
...
extension ProfileVC {
    ...
    func ViewImageApi (param:[String:Any],key:String) {
            ...
            case .success(let result):
                SwiftLoader.hide()
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.profileImage = key//result.data?.preSignedUrl ?? "profile"
                        kUserDefaults.set(result.data?.preSignedUrl ?? "profile", forKey: AppKeys.profile)
                        self.imgUserImage.sd_setImage(with:URL(string: result.data?.preSignedUrl ?? "profile"), placeholderImage: UIImage(named: "user"))
                    }
