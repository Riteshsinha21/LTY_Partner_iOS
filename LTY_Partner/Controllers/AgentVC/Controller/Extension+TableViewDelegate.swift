...
extension AgentVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if dob != "" {
            cell.labelYear.text = "\(self.calcAge(birthday: dob))" + " " + "Years".localized()
        }
...
extension AgentVC : SendEditAgentData{
    func sendEditData(sendValue: String,checkBlockStatus: String) {
        ...
        case "Block" :
            let index = indexValue
            if agentListData[index].partnerState ?? ""  == "ACTIVE" || agentListData[index].partnerState ?? ""  == "ACTIVE".localized() {
//                type = "BLOCKED"
