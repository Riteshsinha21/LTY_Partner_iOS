...
class ProposalVC: UIViewController, UITextFieldDelegate , reCallApiFuncDelegate {
    
    func callApiFunc(setValue: Bool) {
        if setValue == true {
            proposalListArr.removeAll()
    ...
    override func viewDidLoad() {
        ...
        if self.offerType == "PERSONALIZED" {
            setupPersonalizedView()
        }else{
            setupGeneralizedView()
        }
        
//        self.ApiCallingFunc(state: "", offerType: self.offerType)
        
        //Add observer for update mobile and email
        NotificationCenter.default.addObserver(self, selector: #selector(handlesNotification(_:)), name: NSNotification.Name(rawValue: "CallProposalListApi"), object: nil)
  ...
  //MARK:- Api call func
    ...
    func ApiCallingFunc(state:String, offerType:String) {
        ...
        let param:[String: Any] = ["entityType": entityType,"entityId": partnerId, "proposalState": state, "pageSize": 10, "pageCount": self.pageCount, "offerType": offerType]
        proposalVM.proposalListApi(param: param, showLoader: self.pageCount > 1 ? false : true)
    ...
    func setProposalList(data: ProposalModel) {
        if data.status == "SUCCESS"{
            ...
            if self.proposalListArr.count > 0 {
                fetchMoreData = data.data?.proposalsList?.count == 10 ? true : false
...
//        cell.cellPartnerLbl.text = info.policyDetails?.par ?? ""
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
            if indexPath.row == self.proposalListArr.count - 1, fetchMoreData {
                self.pageCount += 1
                self.ApiCallingFunc(state: self.proposalState, offerType: self.offerType)
            }
        }
