...
protocol reCallApiFuncDelegate {
...
class ProposalDetailVC: UIViewController {
    ...
    var reCallProposalListApi :reCallApiFuncDelegate?
    let LTY_AppDelegate = UIApplication.shared.delegate as! AppDelegate
    @IBOutlet weak var companyReserveLbl: UILabel!
    @IBOutlet weak var adminCommissionLbl: UILabel!
    @IBOutlet weak var lblCompanyReserve: UILabel!
...
//        }
    ...
    func setLocalization() {
        ...
        self.lblAdminCommission.text = "Admin Commission".localized()
        self.lblCompanyReserve.text = "Company Reserve".localized()
...
//            pushToVC(vc, animated: true)
    ...
    @IBAction func addActionBtn(_ sender: UIButton) {
        ...
        if memberIdArr.count == 0 {
            self.showOkAlertWithHandler("Since you have changed Agent, Please update the customer also.".localized()) {
                let vc  = FirstViewController.instantiate(fromAppStoryboard: .proposalStoryboard)
                vc.isComingFrom = self.fromWhichVC
                vc.delegate = self
                vc.updateDelegate = self
        ...
        let policyDetails = addProposalDic["policyDocs"] as! [policyDocDetails]
        self.policyDocsArr.removeAllObjects()
        for item in policyDetails{
            let dic = NSMutableDictionary()
            let itemDic = item as! policyDocDetails
          //  print(itemDic)
...
//        let partnerGross = self.commissionData["partnerGrossCommission"] ?? 0.0
        ...
        companyReserveLbl.text = "\(self.commissionData?.companyReserve ?? 0.0)"
        adminCommissionLbl.text = "\(self.commissionData?.adminCommission ?? 0.0)"
...
//        commissionDic.setValue(self.commissionData?.agentCommission ?? 0.0, forKey: "agentCommission")
        ...
        commissionDic.setValue(companyReserveLbl.text, forKey: "companyReserve")
        commissionDic.setValue(adminCommissionLbl.text, forKey: "adminCommission")
                ...
                case .success(let result):
                    ...
                    DispatchQueue.main.async {
                          if result.error?.errorCode == AppKeys.invalidTokenCode {
                              ...
                              self.LTY_AppDelegate.setRootVC()
                          }else {
...
//        let newOfferId = offerId + "_1"
        ...
        commissionDic.setValue(data.commissionDetails?.companyReserve ?? 0, forKey: "companyReserve")
        commissionDic.setValue(data.commissionDetails?.adminCommission ?? 0, forKey: "adminCommission")
...
//        let commissionDetails = addProposalDic["commissionDetails"] as! NSMutableDictionary
        ...
        companyReserveLbl.text = "\(String(describing: data.commissionDetails?.companyReserve ?? 0))"
        adminCommissionLbl.text = "\(String(describing: data.commissionDetails?.adminCommission ?? 0))"
...
//        lblMemberShip.text = membersDetails.componentsJoined(by: ",")
        ...
        for item in policyDetails{
          ...
          //  print(itemDic)
            dic.setValue(itemDic.key, forKey: "key")
            dic.setValue(itemDic.originalFileName, forKey: "originalFileName")
            let modelDic = policyDocDetails.init(key: itemDic.key!, originalFileName: itemDic.originalFileName!)
            self.policyDocsArr.add(modelDic)
        }
        //print(policyDocsArr)
        //self.policyDocsArr = policyDetails
...
//            self.navigationController?.popViewController(animated: true)
            ...
            navigationController?.popViewControllerWithHandler(animated: true, completion: {
                self.reCallProposalListApi?.callApiFunc(setValue: true)
            })
...
//            UserDefaults.standard.removeObject(forKey: "fromLead")
            ...
            if fromWhichVC == "ProposalVC" {
                ...
                let userInfo: [AnyHashable : Any] = ["CallApi":"true"]
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "CallProposalListApi"), object: nil, userInfo: userInfo)
            }
