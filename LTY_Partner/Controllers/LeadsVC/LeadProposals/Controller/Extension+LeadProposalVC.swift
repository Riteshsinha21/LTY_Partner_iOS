...
//        }
        ...
        if info.proposalState == "CLOSED" || info.proposalState == "SIGNED_DOCS" || info.proposalState == "POLICY_CREATED" || info.proposalState == "REJECTED" {
            menuArr = ["View".localized()]
                }else{
                    menuArr =  ["View".localized(),"Edit".localized(),]//["View","Edit","Delete"]
                }
        for cat in menuArr {
            let action = UIAlertAction(title: cat, style: .default) { (action) in
                
                if cat == "View" || cat == "Edit" || cat == "View".localized() || cat == "Edit".localized(){
...
//                        UserDefaults.standard.set(sender.tag == 102 ? "yes" : "no", forKey: "fromLead")
                        vc.fromWhichVC = cat == "View" || cat == "View".localized() ? "LeadProposalVC" : "ForEditProposal"
                        UserDefaults.standard.set(cat == "Edit" || cat == "Edit".localized() ? "yes" : "no", forKey: "fromLead")
                    ...
                    if (checkAccess.1 && userType == "AGENT" && (cat == "Edit" || cat == "Edit".localized())) || (userType == "PARTNER")  || (checkAccess.0 && userType == "AGENT" && (cat == "View" || cat == "View".localized())){
