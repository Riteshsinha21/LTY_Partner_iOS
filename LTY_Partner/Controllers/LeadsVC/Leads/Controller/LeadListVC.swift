...
class LeadListVC: UIViewController, CustomerListProtocolDelegate , updateLeadListApiProtocol {
    func callApiFunc(setValue: String) {
        if setValue == "true" {
            leadListArr.removeAll()
            ApiCallingFunc(status: self.statusCat)
        }
    }
...
//            ApiCallingFunc(status: self.statusCat)
  ...
  //MARK:- Api call func
    ...
    func ApiCallingFunc(status:String) {
        ...
        let param: [String : Any] = [ "entityType": entityType, "entityId": partnerID as Any, "status": status, "pageSize": 10, "pageCount": self.pageCount]
        
      
        leadListViewModel.leadListApiCall(param:param, showLoader: self.pageCount > 1 ? false : true)
    }
    ...
    @IBAction func onClickAllProposal(_ sender: UIButton) {
        ...
        for cat in AllProposalArr {
            ...
            let action = UIAlertAction(title: cat, style: .default) { (action) in
                ...
                if cat == "ALL"{
                    self.statusCat = ""
                    self.ApiCallingFunc(status: self.statusCat)
                }else {
                    self.statusCat = cat
                    self.ApiCallingFunc(status: self.statusCat)
                }
...
//        cell.date.text = returnDOB(date: createdOnStr)
        cell.date.text = utcToLocal(dateStr: createdOnStr)
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
            if indexPath.row == self.leadListArr.count - 1, fetchMoreData {
                self.pageCount += 1
                self.ApiCallingFunc(status: self.statusCat)
            }
        }
...
//            UserDefaults.standard.setValue(try? PropertyListEncoder().encode(memberIds), forKey: "memberIds")
          ...
          //  let vc = BusinessInuranceOfferVC.instantiate(fromAppStoryboard: .Forms)
          // /*
...
//            UserDefaults.standard.setValue(try? PropertyListEncoder().encode(memberIds), forKey: "memberIds")
        ...
        }else if insuranceType == "BUSINESS_INSURANCE_OFFER" || insuranceType == "BUSINESS_INSURANCE_OFFER".localized(){
  ...
  //            UserDefaults.standard.setValue(try? PropertyListEncoder().encode(memberIds), forKey: "memberIds")
            ...
            let info = leadListArr[index]
            vc.updateLeadListApiDelegate = self
            vc.insurnceType = info.metadata?.type ?? ""
            vc.leadId = info.leadId ?? ""
            vc.agentName = info.agentName ?? ""
