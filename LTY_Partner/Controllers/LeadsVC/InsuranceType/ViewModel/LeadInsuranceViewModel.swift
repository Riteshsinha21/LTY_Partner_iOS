...
protocol BusinessOfferProtocolData{
...
class LeadInsuranceViewModel {
    ...
    var businessOfferDelegate :BusinessOfferProtocolData?
    ...
    func businessOfferApiCall(param:[String:Any]){
        SwiftLoader.show(animated: true)
        
        Services.shareInstance.postRequestApi(url: LTY_BASE_URL.BASE_URL_INSURANCE + LTY_END_POINT_URL.viewLeadDetail, method: "POST", passToken: true, expecting:BusinessOfferModel.self, dataDict: param) { response
            ...
            case .success(let result):
                ...
                self.businessOfferDelegate?.setData(data: result)

            case .failure(let error):
                SwiftLoader.hide()
                self.businessOfferDelegate?.showMessage(msg: error.localizedDescription)
